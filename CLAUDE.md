...
code; 0 prints the message as `INFO:`. Keep it at the root of the error, not
under `.context(...)`, or the downcast misses it and the error renders as a
traceback. `ASPECT_DEBUG=1` prints the traceback after the message for anyone
debugging.
