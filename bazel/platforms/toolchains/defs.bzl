...
platforms = [
    struct(exec_os = exec_os, exec_cpu = exec_cpu, tgt_os = tgt_os, tgt_cpu = tgt_cpu)
...
def register_llvm_toolchains():
    for p in platforms:
        native.register_toolchains("//bazel/platforms/toolchains:{}_{}_{}_{}_llvm".format(p.exec_os, p.exec_cpu, p.tgt_os, p.tgt_cpu))
