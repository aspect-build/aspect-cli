...
def multi_platform_brew_artifacts(
    ...
    Args:
        ...
        binaries: The name of the multi_platform_rust_binaries target.
    ...
    bottles = dict()
    for brew_platform, target in binaries.items():
        bottle_name = brews.bottle_name(name, brew_platform)
        bottles[bottle_name] = brew_platform
        brew_bottle(
            name = bottle_name,
            bin_files = [target] + additional_bins,
            bin_renames = {target: "aspect"},
