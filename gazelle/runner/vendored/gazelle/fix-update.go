...
import (
	...
	"github.com/aspect-build/aspect-cli/gazelle/runner/vendored/gazelle/internal/wspace"
...
func runFixUpdate(wd string, languages []language.Language, cmd command, args []string, stats *fixUpdateStatus) error {
	...
	for _, v := range visits {
		...
		if err := uc.emit(v.c, v.file); err != nil {
			if err == errExit || err == resultFileChanged {
				// NOTE: aspect-cli "changed" result, increment counter instead
