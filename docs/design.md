...
Argument types: `args.string()`, `args.int()`, `args.uint()`, `args.boolean()`, their `_list` variants, `args.positional()`, `args.trailing_var_args()`, `args.passthrough(position = ...)`, and `args.custom(type, default = ...)` for non-CLI/config-only values (lambdas, dicts of complex shape, etc.). Scalar/list types support `required`, `default`, `description`, `long` (kebab-flag override), `config_only`, and (for scalar types) `short` for a single-character alias.

`config_only = True` keeps a flag off the command line, leaving it settable from `config.axl` only — for a value a task reads but nobody should type. It cannot be combined with `required`, `short` or `long`, which name or demand a command-line spelling that no longer exists. `args.custom(type, ...)` is config-only by construction and takes no such parameter, and neither do the argv-shaped kinds (`positional`, `trailing_var_args`, `passthrough`), whose content comes from the command line.

Reach for `config_only` over `args.custom` when the value is a list — a `custom` list default cannot be frozen at declaration time and silently resolves to `None` — or simply to keep the declared kind visible (`args.int(default = 3, config_only = True)` rather than `args.custom(int, default = 3)`), reserving `args.custom` for values the CLI genuinely cannot express. Note that a `config.axl` override is not value-checked: `values = [...]` constrains what the CLI accepts, so it has no effect on an arg only config can set.

`args.passthrough()` is how a task that wraps another tool accepts that tool's own flags without redeclaring them — the preferred way to pass a Bazel flag, with `--bazel-flag=` kept as the escape hatch for a flag that collides with one of the task's own: instead of failing an undeclared flag with "unexpected argument", the CLI collects it into the bucket for the slot it was typed in — `position = "pre_command"` for flags before the task name (where Bazel takes its startup options: `aspect --output_base=/tmp/o build`), `position = "post_command"` for flags after it (`aspect build --remote_download_all`). A task may declare one bucket per position, and each arrives as a `list[str]` in command-line order; the built-in Bazel tasks fold them into their startup and command flags respectively. Collected flags are forwarded verbatim — nothing is rewritten. A collected flag claims the token after it only when the bucket's `value_flags_from` sibling arg lists it as taking a separate value (`-c opt`); otherwise that token is left alone, because the arity of a flag nobody declared is unknowable. For the built-in Bazel tasks that list is generated from `bazel help` into `bazel/value_flags.axl` — the union across the latest patch of each supported Bazel major, so a repo pinned to an older Bazel gets the spellings its Bazel accepts — and is replaceable from `config.axl`.

Read a bucket with `ctx.args.claim("<name>")`. Claiming returns the list and records that the task is responsible for those flags, because a flag the user typed to no effect is worse than the parse error it replaced. The runtime enforces it at two moments: before the task spawns the tool it wraps (every `ctx.bazel` invocation checks, so a dropped flag fails in milliseconds rather than after a build), and once more after the implementation returns, as the backstop for a task that spawns nothing. A plain `ctx.args.<name>` read deliberately does not claim — a task may want to inspect the flags before deciding — and a path that runs nothing at all can claim to say the omission was intentional (`bzl.flags.disclaim_passthrough(ctx)` does this for the Bazel tasks). Recognition itself is read off the built clap surface, so any flag declared anywhere in the CLI — including ones clap generates, like `--help` — is never collected; a flag intercepted in Rust before clap parses must still be declared to clap for the same reason.
