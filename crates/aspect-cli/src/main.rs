...
async fn run_shutdown_sequence(signal_name: &str, exit_code: i32) {
    ...
    // Give AXL-spawned children a chance to exit before the SIGKILL escalation.
    child_live::sigterm_all_for_shutdown();
    ...
    if on_recognized_ci() {
        // Let bazel finish its own cleanup so cancellation cannot poison a sandbox.
        if !child_live::live_pids().is_empty() {
            tokio::time::sleep(SIGINT_GRACE).await;
            let killed = child_live::force_kill_all_remaining();
            if killed > 0 {
                errln!("aspect-cli: SIGKILL'd {killed} child process(es) that didn't exit");
            }
        }
    ...
    if killed > 0 {
        errln!("aspect-cli: SIGKILL'd {killed} subprocess(es) that didn't exit");
