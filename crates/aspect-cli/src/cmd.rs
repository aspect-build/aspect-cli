...
pub struct Dispatch {
    pub task_id: usize,
    /// The selected task's kind (`build`, `test`, `mcp`, …) — the deepest
    /// selected subcommand. Unlike `task_name` this is fixed by the task
    /// definition, so it identifies *which* task is running.
...
impl<'a, 'v> Cmd<'a, 'v> {
    ...
    pub fn dispatch(&self, matches: ArgMatches) -> Result<Dispatch, CmdError> {
        ...
        let task_kind = leaf_command_name(&matches).unwrap_or_default();
        ...
        let (task_name, task_name_meaningful) = match explicit_name.or(deprecated_key) {
            Some(name) => (name, true),
            None => {
                let auto = axl_runtime::ci::auto_task_name(&task_kind, generate_name_suffix);
...
mod tests {
    ...
    /// `task_kind` is what identifies *which* task is running (`main.rs` gates
    /// the stdout-telemetry redirect on it). `task_name` cannot serve: it is
    /// auto-suffixed when absent and arbitrary when `--task:name` is given.
    #[test]
    fn dispatch_reports_the_leaf_subcommand_as_the_task_kind() {
        let t = stub_task("mcp", &[], SmallMap::new());
        let cmd = Cmd {
            tasks: vec![&t],
            features: vec![],
            aspect_root: Path::new("/repo"),
        ...
        let matches = root
            .clone()
            .try_get_matches_from(["aspect", "mcp"])
            .expect("parse ok");
        let dispatch = cmd.dispatch(matches).expect("dispatch ok");
        assert_eq!(dispatch.task_kind, "mcp");
        assert_ne!(dispatch.task_name, "mcp");

        let matches = root
            .try_get_matches_from(["aspect", "mcp", "--task:name=renamed"])
            .expect("parse ok");
        let dispatch = cmd.dispatch(matches).expect("dispatch ok");
        assert_eq!(dispatch.task_kind, "mcp");
        assert_eq!(dispatch.task_name, "renamed");
    }
