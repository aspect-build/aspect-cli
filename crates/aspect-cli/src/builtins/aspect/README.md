...
| Strategy        | Fails on                                                           |
|-----------------|--------------------------------------------------------------------|
| `hold-the-line` *(default)* | error-severity diagnostics in **changed** files only |
| `hard`          | any error or linter-process failure                                |
| `soft`          | nothing (diagnostics still rendered)                               |

Diagnostic destination — `--findings-destination=` controlled by `LintTrait` defaults:

- `annotations` — GitHub check-run annotations (one per finding, capped at 50/batch).
- `comments` — GitHub PR review comments.
- `both` *(default on PRs with `LintTrait.findings_destination = "both"`)* — both surfaces.
...
|----------------------|---------------------------------------------------|------------------------------------|--------------|
| `GithubStatusChecks` | [feature/github_status_checks.axl](feature/github_status_checks.axl) | `enabled = True` (default), CI env detected | Creates / updates a GitHub check run via the Aspect App OAuth token. |
| `BuildkiteAnnotations` | [feature/buildkite_annotations.axl](feature/buildkite_annotations.axl) | `enabled = True` (default), `BUILDKITE` env set | Posts `buildkite-agent annotate --scope=job` annotations. |
...
| `BazelDefaults`      | [feature/defaults.axl](feature/defaults.axl)      | always | BK section markers, `--build_metadata=ASPECT_TASK_NAME=…`, env-derived bazel flags. |
...
The two check-run features delegate per-kind rendering through [`lib/check_dispatch.axl`](lib/check_dispatch.axl) so adding a new task kind is a single dispatch-table entry rather than an N×2 update.

## Per-kind result libraries

| Library                                | Public API                                                                        | Used by |
|----------------------------------------|-----------------------------------------------------------------------------------|---------|
| [lib/bazel_results.axl](lib/bazel_results.axl) | `init_results`, `process_event`, `render_check_output`, `compute_reproducer_command`, `resolve_aspect_url`, `build_summary_data`, `build_details_data`, `SHARED_DETAILS_BODY_TEMPLATE` | build, test (and as the base for the others) |
| [lib/lint_results.axl](lib/lint_results.axl) | `init_results`, `accumulate`, `render_check_output`, `compute_annotations`, `chunk_annotations` | lint |
| [lib/format_results.axl](lib/format_results.axl) | `init_results`, `render_check_output`, `format_summary_title` | format |
| [lib/gazelle_results.axl](lib/gazelle_results.axl) | `init_results`, `render_check_output`, `gazelle_summary_title` | gazelle |
| [lib/delivery_results.axl](lib/delivery_results.axl) | `init_results`, `add_result`, `render_check_output`, `delivery_summary_title` | delivery |

Each `*_results.axl` derives its `init_results()` from `bazel_results.init_results()` (so `process_event` can populate the full bazel state) and appends `SHARED_DETAILS_BODY_TEMPLATE` to its task-specific top section so the rendered details body has the same Targets / Build Metrics / Invocation / Workflows Runner / Workspace Status / Build Metadata / Options-parsed sections everywhere.
