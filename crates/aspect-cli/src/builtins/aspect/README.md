...
| `GithubStatusComments`   | [feature/github_status_comments.axl](feature/github_status_comments.axl)   | `enabled = True` (default); any GitHub PR on GitHub Actions, Buildkite, or CircleCI | PR-level *aggregated* sticky comment that rolls every sibling task into one body. Polled and PATCHed in place. Cross-job coordination uses the PR comment's state block on every host, with GitHub Actions / Buildkite artifacts as a freshness optimization (CircleCI is comment-only). |
