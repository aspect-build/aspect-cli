...
classify_flags() {
    ...
    while [[ $i -lt $n ]]; do
        ...
        # name with a leading `no` stripped, so `--nokeep_going` matches the
        # boolean `keep_going`. A few real flags are themselves named `no…`
        # (e.g. `nodep_deps`, `noorder_results`); those still match directly via
        # `bare`, and the `neg` arm only ever adds a match for a genuine
        # `--no<bool>` negation — so the strip can't misroute a flag.
        body="${t#--}"
        bare="${body%%=*}"
        neg="${bare#no}"
        ...
        # Starlark build settings / flags are labels: --//pkg:flag,
        # --@repo//pkg:flag (and --no… for bool_flag negation). They never appear
        # in the harvested native-flag lists — flags-as-proto can't see
        # build-graph-defined settings — and they are always self-contained (bare
        # bool or `=value`; Bazel requires `=`, never a space), so wrap the whole
        # token and never consume the next arg.
        case "$body" in
        //* | @*//* | no//* | no@*//*)
            array_push "$out_name" "${prefix}${t}"
            continue
