...
Before declaring a registry module incompatible with the selected Bazel version, use `bazel mod explain` to inspect its dependency chain. A compatible root dependency may allow module version selection to replace a stale transitive version; confirm compatibility levels and overrides before relying on that remedy.
...
Where a stage substitutes a native library or replaces the global allocator, execute the produced binary as well. That class of defect can pass analysis, compilation, and linking but fail only at run time.
