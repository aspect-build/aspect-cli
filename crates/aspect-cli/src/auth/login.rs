...
pub async fn run_browser(env: AuthEnv) -> miette::Result<ExitCode> {
    ...
    let token_response =
        exchange_code(&client, &auth_code, &redirect_uri, &code_verifier, env).await?;
    ...
    let creds = Credentials {
        ...
        auth_domain: Some(env.domain.to_string()),
        auth_client_id: Some(env.client_id.to_string()),
    };
...
async fn exchange_code(
    ...
    if !resp.status().is_success() {
        ...
        return Err(miette!("token exchange failed (HTTP {}): {}", status, body));
    }
...
pub async fn run_with_api_token(
    client_id: &str,
    secret: &str,
    env: AuthEnv,
...
pub async fn refresh_access_token(creds: &Credentials) -> miette::Result<Credentials> {
    let auth_domain = creds
        .auth_domain
        .as_deref()
        .ok_or_else(|| miette!("no auth_domain stored — cannot refresh"))?;
    let client_id = creds
        .auth_client_id
        .as_deref()
        .ok_or_else(|| miette!("no auth_client_id stored — cannot refresh"))?;
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(miette!("token refresh failed (HTTP {}): {}", status, body));
    }
