...
///
/// When the name is meaningful (an explicit `--task:name` or a CI-job-derived
/// name), mirror the CI status surfaces: lead with the name and bracket the
/// kind as extra context (`<name> [<kind>]`). For a locally-autogenerated
/// placeholder (`<kind>-<random-suffix>`, off CI) the name is noise, so show
/// just `<kind> task`.
fn task_label(kind: &str, name: &str, name_meaningful: bool) -> String {
    if name_meaningful && name != kind {
        format!("{name} [{kind}]")
    } else {
        format!("{kind} task")
    }
...
mod tests {
    ...
    #[test]
    fn task_label_brackets_kind_only_for_meaningful_names() {
        // Meaningful name (explicit --task:name or CI-derived) → `<name> [<kind>]`.
        assert_eq!(
            task_label("format", "format-repeat-2", true),
            "format-repeat-2 [format]"
        );
        assert_eq!(
            task_label("test", "test-ci-linux", true),
            "test-ci-linux [test]"
        );
        // Local autogenerated placeholder → just the kind, no noisy suffix.
        assert_eq!(
            task_label("format", "format-brash-cherries", false),
            "format task"
        );
        // name == kind → no redundant bracket even when "meaningful".
        assert_eq!(task_label("lint", "lint", true), "lint task");
    }
