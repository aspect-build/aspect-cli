...
use starlark::values::{HeapOwner, Value, ValueLike};
use starlark_map::small_map::SmallMap;
use std::path::Path;
use std::path::PathBuf;

use crate::engine::config::feature_context::FeatureContext;
use crate::engine::config::feature_map::construct_features;
use crate::engine::config::trait_map::construct_traits;
...
pub struct ConfigResult {
    ...
    /// Keeps the config heap alive (unfrozen) so all values — including mutable
    /// state dicts in feature closures — remain valid and writable during task execution.
    _heap_owner: HeapOwner,
}
...
impl<'l, 'p> ConfigEvaluator<'l, 'p> {
    ...
    pub fn run_all(
        ...
        // Use HeapOwner so the config heap is never frozen. This keeps all values
        // on the heap — including mutable state dicts in feature closures — writable
        // throughout task execution.
        let heap_owner = HeapOwner::new();
        let (result_tasks, trait_data, feature_data) = {
            let context_module = Module::with_heap(heap_owner.heap());
            ...
            // SAFETY: feature_types live on frozen heaps that outlive this block
            ...
            // Extract trait data directly from the live (unfrozen) TraitMap.
            // SAFETY: heap_owner keeps the config heap alive for the lifetime of ConfigResult.
            let trait_data: Vec<(u64, Value<'static>, Value<'static>)> = trait_map_value
                .downcast_ref::<crate::engine::config::trait_map::TraitMap>()
                .expect("trait_map_value is a TraitMap")
                .entries()
                .into_iter()
                .map(|(id, tv, iv)| {
            ...
            // Extract feature data directly from the live (unfrozen) FeatureMap.
            let feature_data: Vec<(u64, Value<'static>, Value<'static>)> = feature_map_value
                .downcast_ref::<crate::engine::config::feature_map::FeatureMap>()
                .expect("feature_map_value is a FeatureMap")
                .entries()
                .into_iter()
            ...
            (result_tasks, trait_data, feature_data)
            // context_module dropped here; heap_owner keeps memory alive
        };
