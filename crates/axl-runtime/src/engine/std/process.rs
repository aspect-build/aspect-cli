...
pub(crate) fn child_methods(registry: &mut MethodsBuilder) {
    ...
    fn kill<'v>(this: values::Value<'v>) -> anyhow::Result<NoneType> {
        ...
        let inner = inner
            .as_mut()
            .ok_or(anyhow::anyhow!("child is no longer active"))?;
        // Signal the group only while the guard is still held: once `wait` or
        // `try_wait` reaped the leader, its pid may name an unrelated group.
        // The group signal reaches the leader too, so no separate `inner.kill`.
        if child.own_group {
            if child.guard.borrow().is_some() {
                crate::engine::process::sigkill_group(inner.id())
                    .map_err(|e| anyhow!("failed to SIGKILL process group {}: {e}", inner.id()))?;
            }
            return Ok(NoneType);
        }
    ...
    fn terminate<'v>(this: values::Value<'v>) -> anyhow::Result<NoneType> {
        ...
        let inner = inner
            .as_mut()
            .ok_or(anyhow::anyhow!("child is no longer active"))?;
        // Signal the group while the guard is still held (leader not yet
        // reaped): members can outlive the leader, but once `wait`/`try_wait`
        // reaped it, its pid may name an unrelated group.
        if child.own_group {
            if child.guard.borrow().is_some() {
                crate::engine::process::sigterm_group(inner.id())
                    .map_err(|e| anyhow!("failed to SIGTERM process group {}: {e}", inner.id()))?;
            }
            return Ok(NoneType);
        }
...
"#,
    ...
    /// Once `wait()` has reaped an own-group leader its pid may name an
    /// unrelated group, so a later `terminate()`/`kill()` must not signal it.
    #[cfg(unix)]
    #[test]
    fn group_signals_are_noops_after_wait() {
        let exit = crate::test::eval(
            r#"
def _impl(ctx):
    c = ctx.std.process.command("sleep").arg("30").process_group().spawn()
    c.terminate()
    c.wait()
    c.terminate()
    c.kill()
