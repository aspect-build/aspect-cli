...
/// SIGTERM to the whole process group led by `pid`.
#[cfg(unix)]
pub(crate) fn sigterm_group(pid: u32) -> bool {
    use nix::sys::signal::{self, Signal};
    use nix::unistd::Pid;

    signal::killpg(Pid::from_raw(pid as i32), Signal::SIGTERM).is_ok()
}

#[cfg(not(unix))]
pub(crate) fn sigterm_group(_pid: u32) -> bool {
    false
}

/// SIGKILL to the whole process group led by `pid`.
#[cfg(unix)]
pub(crate) fn sigkill_group(pid: u32) -> bool {
    use nix::sys::signal::{self, Signal};
    use nix::unistd::Pid;

    signal::killpg(Pid::from_raw(pid as i32), Signal::SIGKILL).is_ok()
}

#[cfg(not(unix))]
pub(crate) fn sigkill_group(_pid: u32) -> bool {
    false
}
