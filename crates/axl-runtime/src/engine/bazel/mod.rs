...
pub(crate) fn bazel_methods(registry: &mut MethodsBuilder) {
    ...
    /// Run `bazel coverage` over one or more Bazel targets.
    ///
    /// Returns a `Build` object. The call does not block — use `.wait()` to
    /// wait for the invocation to finish and retrieve its exit status.
    ///
    /// `bazel coverage` is the only Bazel verb that triggers the post-test
    /// coverage report generator action, producing the merged LCOV report
    /// at `$(bazel info output_path)/_coverage/_coverage_report.dat` when
    /// `--combined_report=lcov` is passed. `bazel test --collect_code_coverage`
    /// only collects per-test coverage data and does not merge it.
    ///
    /// Argument shape is identical to `ctx.bazel.test` — see that method's
    /// docs for parameter semantics.
    fn coverage<'v>(
        this: values::Value<'v>,
        #[starlark(args)] targets: UnpackTuple<values::StringValue>,
        #[starlark(require = named, default = Either::Left(false))] build_events: Either<
        ...
        let env = Env::from_eval(eval)?;
        let (stdout, stderr) = resolve_stdio(stdio, stdout, stderr)?;
        let coverage = build::Build::spawn(
            "coverage",
            targets.items.iter().map(|f| f.as_str().to_string()),
            build_events,
            execution_log,
