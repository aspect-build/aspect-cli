...
pub(crate) fn subscribe(config: WatchConfig) -> Result<Box<dyn Subscription>, WatchError> {
    ...
    let mut children = Vec::with_capacity(config.roots.len());
    for (i, root) in config.roots.iter().enumerate() {
        match subscribe_root(
            root,
            &format!("fswatch-{}-{i}", std::process::id()),
            &config,
            &pending,
            &error,
            &shutdown,
            ...
            Err(e) => {
                // Don't leak the persistent watchman children already spawned
                // for earlier roots — Child does not kill on drop.
...
fn subscribe_root(
    ...
    if let Err(e) = read_subscribe_ack(&mut reader) {
        let _ = child.kill();
        let _ = child.wait();
        return Err(e);
    }
...
fn read_subscribe_ack(reader: &mut impl BufRead) -> Result<(), WatchError> {
...
fn read_events(
    ...
    for line in stdout.lines() {
        ...
        // Non-unilateral PDUs (e.g. the subscribe acknowledgement) carry no
        // subscription field.
