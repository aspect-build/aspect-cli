...
fn drain_discards_pending(backend: Backend) {
    ...
    // Drain immediately, while the event may still be queued in the backend.
    sub.drain();
    std::thread::sleep(Duration::from_millis(500));
...
#[test]
fn notify_drain_discards_in_flight_events() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path().canonicalize().unwrap();
    let mut sub = watch(Backend::Notify, quick_config(root.clone())).unwrap();
    settle(&mut sub);
    fs::write(root.join("noise.txt"), "x").unwrap();
    sub.drain();
    std::thread::sleep(Duration::from_millis(500));
    let batch = sub.try_recv().unwrap();
    assert!(batch.is_none(), "{batch:?}");
}

#[cfg(unix)]
#[test]
fn notify_preserves_symlinked_root_spelling() {
    let dir = tempfile::tempdir().unwrap();
    let actual = dir.path().join("actual");
    let root = dir.path().join("root-link");
    fs::create_dir(&actual).unwrap();
    fs::create_dir(actual.join("ignored")).unwrap();
    std::os::unix::fs::symlink(&actual, &root).unwrap();

    let mut config = quick_config(root.clone());
    config.ignore_prefixes = vec![root.join("ignored")];
    let mut sub = watch(Backend::Notify, config).unwrap();
    settle(&mut sub);
    fs::write(actual.join("ignored/noise.txt"), "x").unwrap();
    fs::write(actual.join("kept.txt"), "x").unwrap();
    let batch = recv_batch(&mut sub);
    ...
    assert!(
        !batch
            .changes
            .iter()
            .any(|c| c.path.ends_with("ignored/noise.txt")),
        "{batch:?}"
    );
}
