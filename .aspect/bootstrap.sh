    ...
    if [ -n "${BUILDKITE:-}" ]; then
        ...
        if [ -n "${CIRCLE_PROJECT_USERNAME:-}" ] && [ -n "${CIRCLE_PROJECT_REPONAME:-}" ]; then
            ...
            *github*)
                ...
                ;;
            *bitbucket*)
                METADATA_ARGS+=("--build_metadata=VCS=BITBUCKET")
...
if [ -n "${ASPECT_WORKFLOWS_RUNNER:-}" ]; then
    ...
    if [ -z "${ASPECT_WORKFLOWS_RUNNER_NO_LEGACY_CLI:-}" ]; then
        ...
        LOCK_VERSION_FLAG="--aspect:lock_version"
    fi
fi
...
# shellcheck disable=SC2086
bazel $DISABLE_PLUGINS_FLAG $LOCK_VERSION_FLAG $BAZEL_STARTUP_OPTS build $BAZEL_BUILD_OPTS \
...
if [ -n "${GITHUB_ENV:-}" ]; then
    {
        ...
        echo "LOCK_VERSION_FLAG=${LOCK_VERSION_FLAG}"
    } >>"${GITHUB_ENV}"
fi
